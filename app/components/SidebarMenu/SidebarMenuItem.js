...
const SidebarMenuItemLink = (props) =>
	props.to || props.href ? (
		props.to ? (
			<Link to={props.to} className={`${props.classBase}__entry__link`} style={{ fontWeight: "600" }}>
				{props.children}
			</Link>
		) : (
			<a
		...
		<a href="#" className={`${props.classBase}__entry__link`} onClick={() => props.onToggle()} style={{ fontWeight: "600" }}>
			{props.children}
		</a>
	);
				...
				>
					{this.props.icon &&
						React.cloneElement(this.props.icon, {
							className: classNames(this.props.icon.props.className, `${classBase}__entry__icon`),
						})}
					{typeof this.props.title === "string" ? <span>{this.props.title}</span> : this.props.title}
				</SidebarMenuItemLink>
				{this.props.children && (
